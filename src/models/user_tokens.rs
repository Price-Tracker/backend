...
impl UserToken {
    ...
    pub fn generate_refresh_token(conn: &mut PgConnection, user: User) -> String {
        ...
        diesel::insert_into(user_tokens).values(user_token).execute(conn).unwrap();

        token
    }

    pub fn find_refresh_token(conn: &mut PgConnection, token: String) -> QueryResult<UserToken> {
        user_tokens.filter(refresh_token.eq(token)).get_result::<UserToken>(conn)
    }

    pub fn refresh_tokens(conn: &mut PgConnection, user_token: UserToken, config: Data<Config>) -> UserTokensDTO {
        let user = User::find_user_by_id(conn, user_token.user_id)
            .expect("Undefined behavior on the DB side");

        diesel::delete(user_tokens.filter(refresh_token.eq(user_token.refresh_token))).execute(conn).unwrap();
